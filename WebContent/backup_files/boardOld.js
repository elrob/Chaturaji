...
//        });
        ...
        newMsgBox.msgBackground.tween = new Kinetic.Tween({
        ...
        newMsgBox.msgText.tween = new Kinetic.Tween({
        ...
        newMsgBox.msgBackground.tween.play();
        newMsgBox.msgText.tween.play();
    ...
    function msgTextClicked(msgText_ID) {
        ...
        document.body.style.cursor = 'auto';
        clickedMsgBox.msgText.tween = new Kinetic.Tween({
        ...
        clickedMsgBox.msgBackground.tween = new Kinetic.Tween({
        ...
        clickedMsgBox.msgBackground.tween.play();
        clickedMsgBox.msgText.tween.play();
...
//            }
//        });
        movingPiece.tween = new Kinetic.Tween({
        	node: movingPiece,
        	//piecesGroup.get('#' + getSquareFromSquareId(fromSqId).pieceOnId)[0].transitionTo({
        	x: moveToX,
        	//(boardSquareGroup.get('#' + toSqId)[0].getX() + (SQUARE_SIZE / 2)),
    ...
    function process_a_msg() {
        if (!msgDisplayFlag && MSG_BUFFER[0]) {
...
/*    
            ...
            case 13:
                ...
                break;
            default:
                //alert("An unexpected message was received of type: " + receivedMsg.type);
            }
    ...
    function runPreGameTests() {
        ...
        if (pC <= 4) {
            window.setTimeout(runPreGameTests, 2000);
        }
    }
    ...
    function runTests() {
        ...
        if (msgNo < 93) {
            if (!msg) {
                window.setTimeout(runTests, 10000);
            } else if (msg.type === 2) {
                window.setTimeout(runTests, 500);
            } else {
                window.setTimeout(runTests, 500);
            }
        } else {
